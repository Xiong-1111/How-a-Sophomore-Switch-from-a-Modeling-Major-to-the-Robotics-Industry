...
PLACE_OFFSET = np.array([0.0, -0.30, 0.0])#放置点相对于抓取点的相对偏移量（向 Y 轴负方向移动 30cm）
TARGET_CUBE_SIZE = 0.0515
END_EFFECTOR_OFFSET = np.array([0.0, 0.005, 0.0]) #末端在右指上，目标点沿 Y 挪 5 mm，让两指包住方块
EVENTS_DT = [0.008, 0.005, 0.02, 0.02, 0.02, 0.008, 0.0025, 0.05, 0.008, 0.08] #控制器 10 个阶段每步推进多少。数字越小，该阶段持续越久。第 3 段（闭合）和第 4 段（抬起）比默认慢，避免还没夹紧就抬


def get_world_aabb_size(prim_path: str) -> np.ndarray: #获取方块的包围盒大小
...
def prepare_pick_cube(prim_path: str) -> np.ndarray:
    ...
    if not stage.GetPrimAtPath(material_path):
        ...
        material_api.CreateRestitutionAttr(0.0)#弹性设置为0
    UsdShade.MaterialBindingAPI.Apply(prim).Bind(UsdShade.Material(stage.GetPrimAtPath(material_path)))
    ...
    if reach > 0.75 or reach < 0.20:
        print(f"Cube XY reach {reach:.3f} m is outside a reliable workspace; moving to (0.40, 0.20).")
        position[0], position[1] = 0.40, 0.20 #如果方块超出可靠工作空间，移动到(0.40, 0.20)
...
def get_or_create_robot(world: World) -> Franka:
    ...
    for dof_index in robot.gripper.joint_dof_indicies:
        if dof_index is not None:
            robot.get_articulation_controller().switch_dof_control_mode(dof_index=dof_index, mode="position")
    return robot
...
pick_place_cb_id = SimulationManager.register_callback(on_physics_step, IsaacEvents.POST_PHYSICS_STEP)
